...
Route::middleware('authentication')->group(function () {
    ...
    Route::prefix('official-documents')->group(function () {
        ...
        Route::get('/', [OfficialDocumentsController::class, 'getOfficialDocuments'])->middleware('permission:LEER_OFICIOS')->middleware('permission:LEER_OFICIOS');
        //ruta para obtener un oficio por id
        Route::get('/{id}', [OfficialDocumentsController::class, 'getOfficialDocumentById']);
        //lista de archivados
        Route::get('/archived/list', [OfficialDocumentsController::class, 'getArchivedOfficialDocuments']);
        Route::get('/archived/search/term/{term?}', [OfficialDocumentsController::class, 'searchArchivedOfficialDocumentsByTerm']);
        //ruta para obtener una lista de oficios por termino
        Route::get('/search/term/{term?}', [OfficialDocumentsController::class, 'searchOfficialDocumentsByTerm'])->middleware('permission:LEER_OFICIOS');
        //ruta para obtener una lista de oficios por termino
        Route::get('/filter/status/{status}', [OfficialDocumentsController::class, 'filterOfficialDocumentsByStatus'])->middleware('permission:LEER_OFICIOS');
        //ruta para crear un oficio
        Route::post('/create', [OfficialDocumentsController::class, 'createOfficialDocument'])->middleware('permission:CREAR_OFICIOS');
        //ruta para actualizar un oficio
        Route::patch('/update/{id}/estado', [OfficialDocumentsController::class, 'updateOfficialDocument'])->middleware('permission:CANCELAR_OFICIOS|APROBAR_OFICIOS');
        //ruta para archivar un oficio
        Route::put('/archive/{id}', [OfficialDocumentsController::class, 'archiveOfficialDocument'])->middleware('permission:ARCHIVAR_OFICIOS');
        //ruta para restaurar un oficio
        Route::put('/restore/{id}', [OfficialDocumentsController::class, 'restoreOfficialDocument'])->middleware('permission:RESTAURAR_OFICIOS');
        //ruta para eliminar un oficio
        Route::put('/delete/{id}', [OfficialDocumentsController::class, 'deleteOfficialDocument'])->middleware('permission:ELIMINAR_OFICIOS');
        //ruta para compartir un oficio
        Route::post('/share/{id}', [OfficialDocumentsController::class, 'shareOfficialDocument'])->middleware('permission:COMPARTIR_OFICIOS');
    });

    //Files
    Route::prefix('files')->group(function () {

        Route::middleware('permission:LEER_ARCHIVO')->group(function () {
            Route::get('/', [FilesController::class, 'getFiles']);
            Route::get('/{id}', [FilesController::class, 'getFileById']);
        });

        Route::middleware('permission:SUBIR_ARCHIVO')->group(function () {
            Route::post('/upload/{id}', [FilesController::class, 'uploadFiles']);
        });

        Route::middleware('permission:DESCARGAR_ARCHIVO')->group(function () {
    ...
    Route::prefix('comments')->group(function () {
        ...
        Route::middleware('permission:LEER_COMENTARIO')->group(function () {
        ...
        Route::middleware('permission:CREAR_COMENTARIO')->group(function () {
            Route::post('/create/{official_document_id}', [CommentsController::class, 'createComment']);
        });
    });
});
